...
class AccountPayment(models.Model):
    ...
    def _compute_total_receivable(self):
        for order in self:
            if order.partner_id and order.partner_id.credit:
                order.update({'total_receivable': order.partner_id.credit})
            elif order.partner_id and order.partner_id.parent_id and order.partner_id.parent_id.credit:
                order.update({'total_receivable': order.partner_id.parent_id.credit})
            else:
                order.update({'total_receivable': 0.0})
...
class PurchaseOrder(models.Model):
    ...
    def _compute_total_payable(self):
        for order in self:
            if order.partner_id and order.partner_id.debit:
                order.update({'total_payable': order.partner_id.debit})
            else:
                order.update({'total_payable': 0.0})
