...
class KsStockMoveLine(models.Model):
    ...
    ks_product_id = fields.Many2one('product.product', 'Product', ondelete="cascade", check_company=True,
                                    domain="[('type', '!=', 'service'), "
                                           "'|', ('ks_company_id', '=', False), ('ks_company_id', '=', ks_company_id)]")
    ...
    ks_lot_id = fields.Many2one('stock.lot', 'Lot/Serial Number',
                                domain="[('product_id', '=', ks_product_id)]")
    ks_location_dest_id = fields.Many2one('stock.location', 'Destination Location')
    ks_receiver_company_id = fields.Many2one('res.company', related='ks_move_id.ks_receiver_company_id')
    ks_receiver_lot_id = fields.Many2one('stock.lot', string='Receivers Lot',
                                         domain="[('product_id', '=', ks_product_id)]")

    @api.depends('ks_product_id', 'ks_product_uom_id', 'ks_product_uom_qty')
    def ks_compute_product_qty(self):
        for ks_line in self:
            ks_line.ks_product_qty = ks_line.ks_product_uom_id._compute_quantity(ks_line.ks_product_uom_qty,
                                                                                 ks_line.ks_product_id.uom_id,
        ...
        if self.ks_product_id.tracking == 'serial':
            ...
            if self.ks_lot_id:
                ...
                if self.ks_lot_id:
                    ...
                    if self.ks_product_id.tracking == 'serial':
                        if len(ks_move_lines_to_check.filtered(
                                lambda move: move.ks_lot_id.id == self.ks_lot_id.id and move.ks_location_id.id != False)) > 1:
                            self.ks_lot_id = False
                            raise ValidationError(_('You cannot use the same serial number twice. '
                                                    'Please Use different serial numbers.'))
        return res
    ...
    def ks_get_similar_move_lines(self):
        ...
        if ks_transfer_id:
            lines |= ks_transfer_id.ks_move_line_ids.filtered(
                lambda ks_move_line: ks_move_line.ks_product_id == self.ks_product_id and (ks_move_line.ks_lot_id))
        return lines
    ...
    def create(self, vals_list):
        for vals_dict in vals_list:
            if vals_dict:
                if vals_dict.get('ks_move_id'):
                    move_record = self.env['multicompany.transfer.stock.line'].browse(vals_dict['ks_move_id'])
                    if move_record.exists():
                        if not vals_dict.get('ks_location_id', False):
                            vals_dict[
                                'ks_location_id'] = move_record.ks_location_id.id if move_record.ks_location_id else False
                        vals_dict[
                            'ks_company_id'] = move_record.ks_company_id.id if move_record.ks_location_id.company_id else False
                        vals_dict[
                            'ks_product_id'] = move_record.ks_product_id.id if move_record.ks_product_id else False
                        vals_dict[
                            'ks_product_uom_id'] = move_record.ks_product_uom_type.id if move_record.ks_product_uom_type else False
                        vals_dict[
                            'ks_transfer_id'] = move_record.ks_multicompany_transfer_id.id if move_record.ks_multicompany_transfer_id else False
                        vals_dict[
                            'ks_location_dest_id'] = move_record.ks_location_dest_id.id if move_record.ks_location_dest_id else False
    ...
    def _should_bypass_reservation(self, location):
        self.ensure_one()
        if location:
            return location.should_bypass_reservation() or not self.ks_product_id.is_storable
