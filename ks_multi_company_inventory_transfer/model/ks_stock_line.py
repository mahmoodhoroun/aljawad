...
class KsStockTransferMultiCompanyLines(models.Model):
    ...
    ks_location_id = fields.Many2one('stock.location', 'Source Location',
                                     related='ks_multicompany_transfer_id.ks_transfer_from_location')

    ks_location_dest_id = fields.Many2one('stock.location', 'Destination Location',
                                          related='ks_multicompany_transfer_id.ks_transfer_to_location')
    ks_move_line_ids = fields.One2many('multicompany.transfer.move.line', 'ks_move_id', string='Move Lines')

    ks_company_id = fields.Many2one('res.company', related='ks_multicompany_transfer_id.ks_transfer_from')
    ks_receiver_company_id = fields.Many2one('res.company', related='ks_multicompany_transfer_id.ks_transfer_to')
    ks_product_cost = fields.Float('Cost', related='ks_product_id.standard_price')
    ks_product_categ_id = fields.Many2one('product.category', related='ks_product_id.categ_id', string="Category")
    ...
    def ks_compute_reserved_availability(self):
        ...
        for rec in self:
            if rec.state == 'posted':
                rec.ks_reserved_availability = 0
            else:
        ...
        for move in self:
            ...
            if move._should_bypass_reservation():
                ...
                if move.ks_product_id.tracking == 'serial':
                    ...
                    to_update = move.ks_move_line_ids.filtered(
                        lambda move_line: move_line.ks_product_uom_id == move.product_uom and
    ...
    def _should_bypass_reservation(self):
        self.ensure_one()
        return self.ks_multicompany_transfer_id.ks_transfer_from_location.should_bypass_reservation() or not self.ks_product_id.is_storable
    ...
    def _update_reserved_quantity(self, need, available_quantity, location_id, lot_id=None, package_id=None,
        ...
        if not strict:
            taken_quantity_move_uom = self.ks_product_id.uom_id._compute_quantity(taken_quantity,
                                                                                  self.ks_product_uom_type,
                                                                                  rounding_method='DOWN')
            taken_quantity = self.ks_product_uom_type._compute_quantity(taken_quantity_move_uom,
        ...
        for reserved_quant, quantity in quants:
            reserved_quant._update_reserved_quantity(reserved_quant.product_id, reserved_quant.location_id,
                                                     quantity=quantity,
                                                     lot_id=reserved_quant.lot_id)
            to_update = self.ks_move_line_ids.filtered(
                lambda move_line: move_line._reservation_is_updatable(quantity, reserved_quant))
            if to_update:
                to_update[0].with_context(
                    bypass_reservation_update=True).ks_product_uom_qty += self.ks_product_id.uom_id._compute_quantity(
                    quantity, to_update[0].ks_product_uom_id, rounding_method='HALF-UP')
            else:
                if self.ks_product_id.tracking == 'serial':
                    for i in range(0, int(quantity)):
                        self.env['multicompany.transfer.move.line'].create(
                            self.ks_prepare_move_line_vals(quantity=1, reserved_quant=reserved_quant))
                else:
                    self.env['multicompany.transfer.move.line'].create(
                        self.ks_prepare_move_line_vals(quantity=quantity, reserved_quant=reserved_quant))
