...
class res_partner(models.Model):
    ...
    def _compute_cus_account_ids(self):
        for partner in self:
            if partner.filter_by != True:
                partner.cus_account_ids = self.env['account.move'].search([
    ...
    def _compute_cus_overdue_ids(self):
        ...
        for partner in self:
            if partner.filter_by != True:
                partner.cus_overdue_ids = self.env['account.move'].search([
    ...
    def _compute_cus_payment_ids(self):
        for partner in self:
            if partner.filter_by != True:
                ...
                partner.cus_payment_ids = pay_ids
            elif partner.filter_by != False:
                partner.cus_payment_ids = self.env['account.payment'].search([
                    ('partner_id', '=', partner.id),
                    ('partner_type', '=', 'customer'),
                    ('move_id.state', '!=', 'draft'),
                    ('date', '>=', partner.period_from),
                    ('date', '<=', partner.period_to),
                ])
    ...
    cus_account_ids = fields.One2many('account.move', 'partner_id', compute='_compute_cus_account_ids',
                                      string="Customer Statements")
    cus_payment_ids = fields.One2many('account.payment', 'partner_id', compute='_compute_cus_payment_ids',
                                      string="Customer Payments")
    cus_overdue_ids = fields.One2many('account.move', 'partner_id', compute='_compute_cus_overdue_ids',
    ...
    ageing_length = fields.Integer('Days Length', compute='_compute_cus_account_ids', default=30, readonly=False,
                                   store=True)
    ...
    def action_customer_statement_print_without_dues(self):
        if self.period_from > self.period_to:
            raise UserError(_('The start date must be anterior to the end date.'))
        return self.env.ref('gt_customer_account_statement.print_customer_statement_report').with_context(without_dues=True).report_action(self)

    def action_customer_statement_send(self):
        '''
        This function opens a window to compose an email, with the customer statement template message loaded by default.
        '''
        self.ensure_one()
        try:
            template_id = self.env['ir.model.data']._xmlid_to_res_id(
                'gt_customer_account_statement.email_template_customer_statements', raise_if_not_found=False)
        except ValueError:
            template_id = False

        ctx = {
            'default_model': 'res.partner',
            'default_res_ids': self.ids,  # Ensure this is a list of IDs
    ...
    def action_cus_overdue_statement_send(self):
        '''
        This function opens a window to compose an email, with the overdue statement template message loaded by default.
        '''
        self.ensure_one()
        try:
            template_id = self.env.ref('gt_customer_account_statement.email_template_customer_overdue_statements').id
        except ValueError:
            template_id = False
