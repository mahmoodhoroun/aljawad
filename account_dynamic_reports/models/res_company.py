...
class ins_account_financial_report(models.Model):
    ...
    def _get_children_by_order(self, strict_range, processed_ids=None):
        '''returns a recordset of all the children computed recursively, and sorted by sequence. Ready for the printing'''
        # Initialize processed_ids if not provided to prevent infinite recursion
        if processed_ids is None:
            processed_ids = set()
            
        res = self
        # Skip already processed records to prevent infinite recursion
        current_ids = set(self.ids) - processed_ids
        if not current_ids:
            return self.browse()
            
        # Mark current records as processed
        processed_ids.update(current_ids)
        
        # Search for children of non-processed records
        children = self.search([('parent_id', 'in', list(current_ids))], order='sequence ASC')
        if children:
            for child in children:
                # Skip if child is already in result to prevent circular references
                if child.id not in res.ids:
                    res += child._get_children_by_order(strict_range, processed_ids)
                    
        if not strict_range:
            res -= self.env.ref('ins_account_financial_report_unallocated_earnings0')
            res -= self.env.ref('ins_account_financial_report_equitysum0')
        return res
