...
class InsFinancialReport(models.TransientModel):
    ...
    def prepare_where(self):
        ...
        if self.company_id:
            # Include selected company and all its branches (child companies)
    ...
    journal_ids = fields.Many2many('account.journal', string='Journals', required=True,
                                   default=lambda self: self.env['account.journal'].search(
                                       [('company_id', '=', self.company_id.id)]))
    date_from = fields.Date(string='Start Date' , default=_get_default_date_from)
    date_to = fields.Date(string='End Date' , default=_get_default_date_to)
