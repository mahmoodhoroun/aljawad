...
class InsPartnerLedger(models.TransientModel):
    ...
    def _get_default_date_from(self):
        """Return the first day of the current year as default date_from"""
    ...
    def _get_default_date_to(self):
        """Return today's date as default date_to"""
    ...
    account_type = fields.Selection(
        [('peceivable_payable', 'Receivable and Payable'),
         ('any', 'Any'),
         ('asset_receivable', 'Receivable Only'),
         ('liability_payable', 'Payable only')],
        string='Account Type', default='peceivable_payable'
    )
    ...
    date_from = fields.Date(
        string='Start date', default=_get_default_date_from
    )
    date_to = fields.Date(
        string='End date', default=_get_default_date_to
    )
    ...
    def prepare_where(self, mode='strict'):
        ...
        cmpny_ids = self.env.context.get('allowed_company_ids')
        ...
        if self.account_type == 'peceivable_payable':
            where += " AND a.account_type in ('asset_receivable', 'liability_payable')"
    ...
    def prepare_detailed_lines(self, move_line_ids=[], partner_id=False):
        ...
        sql = ('''
            SELECT
                ...
                CASE 
                    WHEN l.payment_id IS NOT NULL THEN 
                        (SELECT memo FROM account_payment WHERE id = l.payment_id)
                    ELSE ''
                END AS payment_memo,
                CASE 
                    WHEN l.payment_id IS NOT NULL THEN 
                        (SELECT bank_reference FROM account_payment WHERE id = l.payment_id)
                    ELSE ''
                END AS payment_bank_ref
            ''' + self.prepare_from() + where_current +
               ''' AND l.partner_id = %(partner_id)s 
               GROUP BY
                   l.date, l.id, m.id, l.matching_number,
                   l.date_maturity, j.code, a.name, l.currency_id, a.code_store, m.name, l.name, l.debit, l.credit, l.amount_currency, l.payment_id
               ORDER BY l.date
           ''')
    ...
    def action_xlsx(self):
        ...
        sheet.write_string(row_pos, 2, _('Memo'), format_header)
        sheet.write_string(row_pos, 3, _('Bank Reference'), format_header)
        sheet.write_string(row_pos, 4, _('Invoice Date'), format_header)
        sheet.write_string(row_pos, 5, _('Due Date'), format_header)
        sheet.write_string(row_pos, 6, _('Account'), format_header)
        sheet.write_string(row_pos, 7, _('Matching'), format_header)
        sheet.write_string(row_pos, 8, _('Debit'), format_header)
        sheet.write_string(row_pos, 9, _('Credit'), format_header)
        sheet.write_string(row_pos, 10, _('Amount Currency'), format_header)
        sheet.write_string(row_pos, 11, _('Balance'), format_header)

        if account_lines:
            for line in account_lines:
                row_pos += 1
                sheet.merge_range(row_pos, 0, row_pos, 7, line.get('partner_name'), line_header_left)
                sheet.write(row_pos, 8, float(line.get('debit')), line_header)
                sheet.write(row_pos, 9, float(line.get('credit')), line_header)
                sheet.write(row_pos, 10, '', line_header)
                sheet.write(row_pos, 11, float(line.get('balance')), line_header)
                ...
                for sub_line in sub_lines:
                    if sub_line.get('ttype') == 'initial':
                        row_pos += 1
                        sheet.write(row_pos, 7, 'Initial Balance', line_header_light_ending_bold)
                        sheet.write(row_pos, 8, float(sub_line.get('debit')), line_header_light_initial)
                        sheet.write(row_pos, 9, float(sub_line.get('credit')), line_header_light_initial)
                        sheet.write(row_pos, 10, '', line_header_light_initial)
                        sheet.write(row_pos, 11, float(sub_line.get('balance')), line_header_light_initial)
                        ...
                        # Write memo (if payment)
                        memo = sub_line.get('payment_memo') or ''
                        sheet.write(row_pos, 2, memo, line_header_light)
                        # Write bank reference (if payment)
                        bank_ref = sub_line.get('payment_bank_ref') or ''
                        sheet.write(row_pos, 3, bank_ref, line_header_light)
                        datestring = fields.Date.from_string(str(sub_line.get('ldate'))).strftime(lang_id.date_format)
                        sheet.write(row_pos, 4, datestring, line_header_light_date)
                        if sub_line.get('date_maturity'):
                            ...
                            sheet.write(row_pos, 5, datestring, line_header_light_date)
                        else:
                            sheet.write(row_pos, 5, datestring, line_header_light_date)
                        sheet.write(row_pos, 6, sub_line.get('account_name'), line_header_light)
                        sheet.write(row_pos, 7, sub_line.get('matching_number') or '', line_header_light)
                        sheet.write(row_pos, 8, float(sub_line.get('debit')), line_header_light)
                        sheet.write(row_pos, 9, float(sub_line.get('credit')), line_header_light)
                        sheet.write(row_pos, 10, float(sub_line.get('amount_currency')), line_header_light)
                        sheet.write(row_pos, 11, float(sub_line.get('balance')), line_header_light)
                    else:  # Ending Balance
                        row_pos += 1
                        sheet.write(row_pos, 7, 'Ending Balance', line_header_light_ending_bold)
                        sheet.write(row_pos, 8, float(sub_line.get('debit')), line_header_light_ending)
                        sheet.write(row_pos, 9, float(sub_line.get('credit')), line_header_light_ending)
                        sheet.write(row_pos, 10, '', line_header_light_ending)
                        sheet.write(row_pos, 11, float(sub_line.get('balance')), line_header_light_ending)
