...
class ProductProduct(models.Model):
    ...
    def write(self, vals):
        mapping_objs = self.mapped('channel_mapping_ids')
        mapping_objs.write({'need_sync': 'yes'})
        self.mapped('product_tmpl_id.channel_mapping_ids').write({'need_sync': 'yes'})
        vals = self.env['multi.channel.sale']._core_pre_post_write(self, 'pre', 'product', mapping_objs, vals)
        result = super(ProductProduct, self).write(vals)
        self.env['multi.channel.sale']._core_pre_post_write(self, 'post', 'product', mapping_objs, vals)
        return result
