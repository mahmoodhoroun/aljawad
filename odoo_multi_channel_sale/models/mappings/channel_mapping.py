...
class ChannelMappings(models.Model):
    ...
    def create(self,vals_list):
        for vals in vals_list:
            if not vals.get('operation') == 'export' and not self._context.get('channel_id'):
                vals.update({'operation': 'manual'})
        return super().create(vals_list)

    def write(self, vals):
        if vals.get('need_sync') and self._context.get('from_multichannel'):
            vals.pop('need_sync')
