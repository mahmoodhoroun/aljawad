...
class PartnerFeed(models.Model):
    ...
    company_name = fields.Char('Company Name')                    #####made field for company name

    def get_partner_fields(self):
        return copy.deepcopy(PartnerFields)
    ...
    def _create_feed(self, partner_data):
        ...
        if contact_data_list or partner_data.get('type') not in ['invoice','delivery']:
...
# & remove this code
    ...
    def import_partner(self, channel_id):
        ...
        vals = EL(self.read(self.get_partner_fields()))
        _type = vals.get('type')
        if not _type == 'contact' or not vals.get('vat'):
            vals.pop('vat')
        if not channel_id.sync_customer_company:
